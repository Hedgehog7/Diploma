...
APlugin::APlugin(Type _type, const std::string & _name, const std::string & _description, const std::string & _ext)
	:m_type(_type), m_name(_name), m_description(_description), m_ext(_ext)
{
}
...
const std::string & APlugin::ext() const
{
	return m_ext;
}
...
AEditorPlugin::AEditorPlugin(const std::string document_extension, const std::string & document_description, const std::string & editor_title, const std::string & _name, const std::string & _description)
	:APlugin(APlugin::Type::Editor, _name, _description, document_extension), m_docExt(document_extension), m_docDescr(document_description), m_editorTitle(editor_title)
{
}
...
void AEditorInstance::saveDocument(std::string path) {
	m_pDocument->save(path);
}
